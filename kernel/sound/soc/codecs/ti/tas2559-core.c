...
static unsigned int p_tas2559_startup_data[] = {
	...
	DevA, TAS2559_GPIO5_PIN_REG, 0x12,/*ASI2 BCLK*/
	DevA, TAS2559_GPIO6_PIN_REG, 0x11,/*ASI2 WCLK*/
	DevA, TAS2559_GPIO7_PIN_REG, 0x10,/*ASI2 DOUT = ASI1 DOUT*/
	DevA, TAS2559_ASI2_DIN_DOUT_MUX_REG, 0x60, //DIN, DOUT
	DevA, TAS2559_ASI2_DAC_CLKOUT_REG, 0x11, //BCLK, WCLK output set
...
void set_tasxxx_amp_type(int type)
{
	amp_type = type; // 0 : TAS5766 , 1 : TAS2559	
}

int get_tasxxx_amp_type(void)
{
	return amp_type; // 0 : TAS5766 , 1 : TAS2559	
}
#endif
...
int tas2559_enable(struct tas2559_priv *pTAS2559, bool bEnable)
{
	...
	/* check safe guard*/
#if 1 // hjkoh TODO: fix safe guard function
	nValue = 0;
#else
	...
	if (nBook == TAS2559_YRAM_BOOK1) {
		if (nPage == TAS2559_YRAM1_PAGE) {
			if (nReg >= TAS2559_YRAM1_START_REG) {
				...
				if ((nReg + len) > TAS2559_YRAM1_START_REG) {
			...
			if (nReg > TAS2559_YRAM3_END_REG) {
				...
				if ((nReg + len) < TAS2559_YRAM3_START_REG) {
					nResult = 0;
				} else {
		...
		if (nPage == TAS2559_YRAM5_PAGE) {
			if (nReg > TAS2559_YRAM5_END_REG) {
				...
				if ((nReg + len) < TAS2559_YRAM5_START_REG) {
					nResult = 0;
				} else {
	...
	if (nBook == TAS2559_YRAM_BOOK1) {
		if (nPage < TAS2559_YRAM2_START_PAGE)
			...
			if (nReg > TAS2559_YRAM2_END_REG) {
				...
				if ((nReg + len) < TAS2559_YRAM2_START_REG) {
					nResult = 0;
				} else {
	...
	if (nBook == TAS2560_YRAM_BOOK) {
		if (nPage < TAS2560_YRAM_START_PAGE) {
			...
			if (nReg > TAS2560_YRAM_END_REG) {
				...
				if ((nReg + len) < TAS2560_YRAM_START_REG) {
					nResult = 0;
				} else {
...
int tas2559_parse_dt(struct device *dev, struct tas2559_priv *pTAS2559)
{
	...
	if (!gpio_is_valid(pTAS2559->mnDevAGPIORST))
		dev_err(pTAS2559->dev, "Looking up %s property in node %s failed %d\n",
			"ti,tas2559-gpio-left", np->full_name, pTAS2559->mnDevAGPIORST);
	else
	...
	if (!gpio_is_valid(pTAS2559->mnDevBGPIORST))
		dev_err(pTAS2559->dev, "Looking up %s property in node %s failed %d\n",
			"ti,tas2560-gpio-right", np->full_name, pTAS2559->mnDevBGPIORST);
	else
	...
	if (!gpio_is_valid(pTAS2559->ldo_enable_gpio))
		...
		dev_dbg(pTAS2559->dev, "%s, tas2559 ldo-enable-gpio %d\n", __func__, pTAS2559->ldo_enable_gpio);
#endif
