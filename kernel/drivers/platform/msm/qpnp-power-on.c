...
/* Prepare PON KPDPWR S2 reset (bite) */ 
int pmic_s2_reset_enable(struct qpnp_pon *pon, u16 s2_cntl2_addr ) 
{ 
    int rc = 0;
    /* enable s2 reset */ 
    rc = qpnp_pon_masked_write(pon, s2_cntl2_addr,
...
/* Disable PON KPDPWR S2 reset. (bite)*/ 
int pmic_s2_reset_disable(struct qpnp_pon *pon, u16 s2_cntl2_addr) 
{ 
    int rc = 0;
    /* disable s2 reset */ 
    rc = qpnp_pon_masked_write(pon, s2_cntl2_addr,
...
static int reset_timer_set_time(struct qpnp_pon *pon, u32 value)
{
    int rc=0;
    struct qpnp_pon_config *cfg = NULL;
    cfg = qpnp_get_cfg(pon, 0);

	mutex_lock(&pon->pon_input->mutex);
    printk("=========== Value : %d \n",value);
    switch(value){
        case 0:
            rc = pmic_s2_reset_disable(pon, cfg->s2_cntl2_addr);
            break;
        case 1:
            rc = pmic_s2_reset_enable(pon, cfg->s2_cntl2_addr);
            break;
        default:
            printk("=============== Error : %d\n", value);
    }
...
static ssize_t reset_timer_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
...
static ssize_t reset_timer_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	...
	rc=reset_timer_set_time(pon, value);
