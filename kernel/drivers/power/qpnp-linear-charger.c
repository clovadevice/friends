...
/* PON Interrupt offsets */
#define PON_INT_RT_STS_REG				0x10
#define CBLPWR_ON_MASK					BIT(2)
...
#define PON_INT_RT_STS_SUBTYPE		0x1
...
#ifndef NO_USE_BATTERY_THERM
static enum power_supply_property msm_batt_therm_power_props[] = {
	POWER_SUPPLY_PROP_BATT_THERM,
	POWER_SUPPLY_PROP_BATT_THERM_ENABLE,
};
#endif
...
static int qpnp_lbc_is_usb_external_chg_plugged_in(struct qpnp_lbc_chip *chip)
{
	u8 cblpwr_on;
	int rc;

	rc = qpnp_lbc_read(chip, chip->pon_int_rt_sts_base + PON_INT_RT_STS_REG,
				&cblpwr_on, 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n",
				chip->pon_int_rt_sts_base + PON_INT_RT_STS_REG, rc);
		return rc;
	}

	pr_debug("rt_sts 0x%x\n", cblpwr_on);

	return (cblpwr_on & CBLPWR_ON_MASK) ? 1 : 0;
}
...
#ifndef NO_USE_BATTERY_THERM
static int get_prop_batt_therm(struct qpnp_lbc_chip *chip)
{
	int rc = 0;
	struct qpnp_vadc_result results;
...
#ifndef NO_USE_BATTERY_THERM
static int qpnp_batt_therm_property_is_writeable(struct power_supply *psy,
					enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_BATT_THERM_ENABLE:
		return 1;
	default:
		break;
	}
...
#ifndef NO_USE_BATTERY_THERM
static int qpnp_batt_therm_power_set_property(struct power_supply *psy,
				enum power_supply_property psp,
				const union power_supply_propval *val)
{
	struct qpnp_lbc_chip *chip = container_of(psy, struct qpnp_lbc_chip,
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_BATT_THERM_ENABLE:
		if(chip->batt_therm_enable != val->intval){			
			chip->batt_therm_enable = val->intval;
			pr_debug("power supply changed batt therm\n");
			changed = 1;
		}
		break;
	default:
	...
	if(changed) power_supply_changed(&chip->batt_psy);
	return 0;
}
#endif
...
#ifndef NO_USE_BATTERY_THERM
static int qpnp_batt_therm_power_get_property(struct power_supply *psy,
				       enum power_supply_property psp,
				       union power_supply_propval *val)
{
	struct qpnp_lbc_chip *chip =
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_BATT_THERM:
		val->intval = get_prop_batt_therm(chip);
		break;
	case POWER_SUPPLY_PROP_BATT_THERM_ENABLE:
		val->intval = chip->batt_therm_enable;
		break;
	default:
		return -EINVAL;
	}
...
#ifndef NO_USE_BATTERY_THERM
	chip->batt_therm_enable = of_property_read_bool(
			chip->spmi->dev.of_node, "batt-therm-poweroff-enabled");
#endif
...
static irqreturn_t qpnp_lbc_cblpwr_on_irq_handler(int irq, void *_chip)
{
	...
	usb_present = qpnp_lbc_is_usb_external_chg_plugged_in(chip);
	pr_debug("usbin-valid triggered: %d\n", usb_present);
...
static int qpnp_lbc_request_irqs_external(struct qpnp_lbc_chip *chip)
{
	int rc = 0;

	SPMI_REQUEST_IRQ(chip, CBLPWR_ON, rc, cblpwr_on, 0,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, 1);
...
/* Get/Set initial state of external charger */
static void determine_initial_external_status(struct qpnp_lbc_chip *chip)
{
	chip->usb_present = qpnp_lbc_is_usb_external_chg_plugged_in(chip);
	power_supply_set_present(chip->usb_psy, chip->usb_present);
	/*
	 * Set USB psy online to avoid userspace from shutting down if battery
	 * capacity is at zero and no chargers online.
...
static void qpnp_lbc_collapsible_detection_work(struct work_struct *work)
{
	...
	qpnp_lbc_enable_irq(chip, &chip->irqs[CBLPWR_ON]);
}
...
static int qpnp_lbc_parse_resources(struct qpnp_lbc_chip *chip)
{
	...
	spmi_for_each_container_dev(spmi_resource, spmi) {
		...
		switch (subtype) {
		case PON_INT_RT_STS_SUBTYPE:
			chip->pon_int_rt_sts_base = resource->start;
			rc = qpnp_lbc_get_irqs(chip, subtype, spmi_resource);
			if (rc) {
				pr_err("Failed to get PON_INT irqs rc=%d\n",
						rc);
				return rc;
			}
			break;
...
external_charger_usb:
	...
	chip->batt_psy.name = "batt_therm";
	chip->batt_psy.properties = msm_batt_therm_power_props;
	chip->batt_psy.type = POWER_SUPPLY_TYPE_THERM;
	chip->batt_psy.num_properties = ARRAY_SIZE(msm_batt_therm_power_props);
	chip->batt_psy.get_property = qpnp_batt_therm_power_get_property;
	chip->batt_psy.set_property = qpnp_batt_therm_power_set_property;
	chip->batt_psy.property_is_writeable =
			qpnp_batt_therm_property_is_writeable;
	
	rc = power_supply_register(chip->dev, &chip->batt_psy);
	if (rc < 0) {
		pr_err("batt therm failed to register rc=%d\n", rc);
	}
#endif
