...
// haksukim : add fg capacity
	...
	if (!chip->fg_psy) {
		pr_err("FG supply not found; defer probe\\n");
		return 0;
	}

	if (chip->fg_psy) {
		chip->fg_psy->get_property(chip->fg_psy, POWER_SUPPLY_PROP_CAPACITY , &ret);
	}

	chip->soc_now =  ret.intval;
#else
	u8 reg;
	u32 temp = 0;
	int rc, soc = 0;
...
static void smb1360_external_power_changed(struct power_supply *psy)
{
	...
	{
		current_limit = chip->info_charge_max_ma;
	}
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	rc = of_property_read_u32(node, "info,charge-max-ma",
						&chip->info_charge_max_ma);
	if (rc < 0)
		chip->info_charge_max_ma = 0;
